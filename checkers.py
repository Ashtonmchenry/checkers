...
class Checker(TwoPlayerGame):
    ...
    def __init__(self, players):
        ...
        self.blank_board = np.zeros((8,8), dtype=object) #makes a blank board. np.zeros makes a list of lists. 8 lists each filled with 8 zeroes
        self.board = self.blank_board.copy() #copy blank board to another attribute
        self.black_pieces = [
            (0,1), (0,3), (0,5), (0,7),     #The black spaces on black's
            (1,0), (1,2), (1,4), (1,6)      #side of the board
        ]
        self.white_pieces = [
            (6,1), (6,3), (6,5), (6,7),     #The black spaces on white's
            (7,0), (7,2), (7,4), (7,6)      #side of the board
        ]
        for i,j in self.black_pieces:       #Placing black pieces on the board
            self.board[i,j] = "B"
        for i,j in self.white_pieces:       #Placing white pieces on the board
            self.board[i,j] = "W"

        self.white_territory = [(7,0), (7,2), (7,4), (7,6)]     #White's back line (coordinates) where black's can become kings
        self.black_territory = [(0,1), (0,3), (0,5), (0,7)]     #Black's back line (coordinates) where white's can become kings
    ...
    def lose(self):
        ...
        for (i,j) in self.black_territory:
            if (self.black_territory == 'W'):
                self.is_over()

            else:
                for (i,j) in self.white_territory:
                    if (self.white_territory == 'B'):
                        self.is_over()
